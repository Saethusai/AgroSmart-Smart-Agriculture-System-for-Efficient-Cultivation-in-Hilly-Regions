...
### Irrigation (Protected)
- `GET /api/irrigation/recommendation` - Get irrigation recommendation
- `POST /api/irrigation/start` - Start irrigation
- `POST /api/irrigation/stop/:id` - Stop irrigation
- `GET /api/irrigation/status` - Get current status
- `GET /api/irrigation/logs` - Get irrigation history
- `GET /api/irrigation/stats` - Get statistics

## 📊 Testing with Mock Data

Since this is a demonstration system, you can simulate sensor data:
...
setInterval(async () => {
  const data = {
    ...
    waterTankLevel: 60 + Math.random() * 40
  };
  
  await axios.post('http://localhost:5000/api/sensors/data', data, {
...
If Twilio is not configured, the system will log SMS messages to the console instead.
